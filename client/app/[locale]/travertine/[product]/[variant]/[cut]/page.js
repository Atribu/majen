...
// — Aynı normalization'ı burada da kullanıyoruz (variant sayfasını bozmuyoruz)
...
// — Yeni: izinli değerler
const CUTS = ["vein-cut", "cross-cut"];               // kesim şekli
const PROCESSES = ["natural", "filling", "epoxy", "transparent", "antique"]; // işleme (bir sonraki adımda linkleyeceğiz)
...
export default async function Page({ params }) {
  ...
  const productSlug = rawProduct; // URL’deki slug'ı koruyoruz
      ...
      <section className="max-w-6xl mx-auto px-6 py-10 md:py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-start">
          ...
          <div className="flex flex-col">
            ...
            <div className="mt-8">
              <h2 className="text-lg md:text-xl font-semibold text-neutral-900 mb-3">Choose process</h2>
              <div className="flex flex-wrap gap-3">
                {PROCESSES.map((p) => (
                  <Link
                    key={p}
                    href={`/${locale}/${base}/${productSlug}/${variantSlug}/${cut}/${p}`}
