...
export default function IntroSection({
  ...
  const _crumbSectionLabel = crumbSectionLabel ?? (locale === "tr" ? "Traverten" : "Travertine");
  const _crumbSectionHref  = crumbSectionHref  ?? `/${locale}/${locale === "tr" ? "traverten" : "travertine"}`;
  ...
  return (
    <section className={className} aria-labelledby="intro-heading">
      ...
      <div className="lg:hidden relative w-full h-[550px] lg:h-[500px] flex items-center justify-center">
        <Image src={bgMobile} alt="Background" fill priority className="object-cover" />
        ...
        <div className="relative z-10 px-2 lg:px-5 text-center text-white max-w-[95%]">
          ...
          {_intro && (<p className="mt-1 lg:mt-3 text-[12px] md:text-[14px] leading-tight">{_intro}</p>)}
          {_title2 && (<h2 className="text-[24px] md:text-[32px] lg:text-[36px] font-semibold mt-3">{_title2}</h2>)}
          {_intro2 && (<p className="mt-1 lg:mt-3 text-[12px] md:text-[14px] leading-tight">{_intro2}</p>)}

          {showBreadcrumb && (
            <nav className="mt-5 inline-flex items-center rounded-lg bg-white/20 backdrop-blur px-3 py-1.5 text-xs sm:text-sm" aria-label="breadcrumb">
      ...
      <div className="hidden lg:flex flex-row items-center justify-start h-[500px] relative">
        ...
        <div className="absolute rounded-sm h-[500px] 2xl:h-[520px] w-[860px] 2xl:w-[50%] 2xl:max-w-[1000px] text-black flex flex-col justify-between left-1/2 -translate-x-1/2 ">
          ...
          <header className="relative px-5 pt-5 w-[80%] z-10">
            <h1 className="text-[28px] md:text-[36px] lg:text-[40px] font-semibold">{_title}</h1>
            {_intro && (<p className="text-[12px] md:text-[14px] lg:text-[18px]">{_intro}</p>)}
            {_title2 && (<h2 className="text-[24px] md:text-[32px] lg:text-[36px] font-semibold mt-3">{_title2}</h2>)}
            {_intro2 && (<p className="text-[12px] md:text-[14px] lg:text-[18px]">{_intro2}</p>)}
          </header>
          ...
          {showBreadcrumb && (
            <nav className="bg-transparent px-4 py-3 rounded-sm text-black z-[999]" aria-label="breadcrumb">
              <ol className="flex flex-wrap items-center gap-x-1 text-sm">
                <li><Link href={_crumbHomeHref} className="hover:underline">{_crumbHomeLabel}</Link></li>
                <li><span className="mx-1 text-black/50">/</span><Link href={_crumbSectionHref} className="hover:underline">{_crumbSectionLabel}</Link></li>
              </ol>
            </nav>
          )}
          ...
          <div className="absolute right-[-500px] top-1/2 -translate-y-1/2 w-[800px] h-[500px] ">
            <Image src={heroSrc} alt={imageAlt} fill className="object-contain" priority />
          </div>
        </div>
      </div>
